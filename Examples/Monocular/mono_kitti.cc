...
int main(int argc, char **argv)
{
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        SLAM.TrackMonocular(im,tframe);//传入图像以及该帧对应的时间戳
        ...
        vTimesTrack[ni]=ttrack;//耗时统计

        // Wait to load the next frame，确保程序处理两针图像之间的时间间隔和数据集的采集时间戳间隔一致
        double T=0;
        if(ni<nImages-1)
...
void LoadImages(const string &strPathToSequence, vector<string> &vstrImageFilenames, vector<double> &vTimestamps)
{
    ...
    {
        stringstream ss;
        //setfill和setw都是io中设置字段输出格式的，只对紧接着的输出有效，也就是将i(比如i=1)设置为000001
