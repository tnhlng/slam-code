...
System::System(const string &strVocFile,					//词典文件路径
               ...
               const bool bUseViewer):						//是否使用可视化界面，默认参数是使用true
        			 ...
        			 mbDeactivateLocalizationMode(false)		//没有这个模式转换标志
{
    ...
    //Check settings file
    //FileStorage类将各种OpenCV数据结构的数据存储为XML 或 YML格式。 yml常用来用作配置文件，FileStorage可以类比为C++中的输入输出流
    ...
    //加载词库，并获取字典加载状态
    bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile);//K叉树词典，深度为L
    //如果加载失败，就输出调试信息
    if(!bVocLoad)
    {
    ...
    //Create KeyFrame Database，关键帧数据库，用于通过词包（或者帧，帧中提取出词包）快速搜索到可能的关键帧
    ...
    //tracking 将作为主线程运行，其他线程将作为后台线程做辅助工作。
    mpTracker = new Tracking(this,						//现在还不是很明白为什么这里还需要一个this指针  TODO--是用于初始化的
    						 mpVocabulary,				//字典
    						 mpFrameDrawer, 			//帧绘制器
    						 mpMapDrawer,				//地图绘制器
                             mpMap, 					//地图
